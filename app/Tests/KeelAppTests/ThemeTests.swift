...
final class ThemeTests: XCTestCase {
    ...
    func testContentGrowingUnderAPaneDoesNotStopItFollowing() {
        ...
        // At the end during a gesture: leave the offset to the person until it settles.
        XCTAssertFalse(FollowsTail.following(false, atBottom: true, byHand: true))
    ...
    func testPrimaryButtonLabelsRemainReadableInBothAppearances() {
        for appearance in [light, dark] {
            for tone in [K.C.accent, K.C.add, K.C.del, K.C.warn] {
                XCTAssertGreaterThan(contrast(resolve(K.C.onAccent, appearance), resolve(tone, appearance)), 4.5)
            }
        }
    }
