...
final class BudgetTests: XCTestCase {
    ...
    /// - `SessionModel` ×6: the monitors debounce (300 ms after the daemon's `monitors.changed`,
    ...
    ///   (`justOpened`, `remembered`, `discarded`) that are timers, not requests. The
    ///   design-change wait lives on `DesignerViewModel` now.
    ...
    func testTheMainPageDoesNotPoll() throws {
        ...
        XCTAssertEqual(sleeps, ["SessionModel.swift": 6, "Lanes.swift": 1, "SidePanel.swift": 0],
