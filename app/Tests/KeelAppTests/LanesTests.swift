...
final class LanesTests: XCTestCase {
    ...
    /// A torn-out window's tabs are its own, and it never touches the saved list.
    ///
    /// The saved list is keyed by project alone, so two windows on one project were two writers of
    /// one key with the last one winning — and, worse, the torn-out window *restored* it, so a lane
    /// dragged into a window of its own arrived to find the other window's tabs over the top of it.
    /// The flag is on `Lanes` rather than on the view because `switchProject` restores as well.
    func testAWindowThatDoesNotRememberNeitherWritesNorReads() async {
        let owner = lanes()
        owner.lanes[0].sessionId = "aaa"
        owner.remember(repo: repo)
        let saved = UserDefaults.standard.data(forKey: "keel.lanes." + repo)
        XCTAssertNotNil(saved, "the window that owns the tabs still writes them")

        let torn = Lanes(client: Client(port: 0), port: 0, remembers: false)
        let its = torn.lanes.map(\.id)
        torn.lanes[0].sessionId = "bbb"
        torn.remember(repo: repo)
        XCTAssertEqual(UserDefaults.standard.data(forKey: "keel.lanes." + repo), saved,
                       "a torn-out window overwrote the other window's saved tabs")

        await torn.restore(repo: repo)
        XCTAssertEqual(torn.lanes.map(\.id), its,
                       "a torn-out window replaced its own lane with the other window's tabs")
    }
