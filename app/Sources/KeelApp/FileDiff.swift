    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            if open, let diff {
                ...
                // Capped: this stack is not lazy, and a card is a preview. The rest is one
                // click away in the pane that reads a whole file lazily.
                let (shown, dropped) = diff.preview()
                ForEach(Array(shown.enumerated()), id: \.offset) { hi, hunk in
                    HunkHeader(header: hunk.header, index: hi, path: path,
                               // The cut lands in the last hunk, and discarding that one would
                               // put back lines this card never showed.
                               discardable: !diff.untracked && diff.note == nil
                                   && !(dropped > 0 && hi == shown.count - 1),
                               model: model)
                ...
                if dropped > 0 {
                    Button("\(dropped) more line\(dropped == 1 ? "" : "s") — open the full diff") {
                        model.show(diff: path)
                    }
                    .buttonStyle(QuietButton(tone: K.C.accent))
                    .frame(maxWidth: .infinity, alignment: .leading)
                    .padding(.horizontal, K.S.md).padding(.vertical, K.S.sm)
                    .help("This card stops at \(shown.reduce(0) { $0 + $1.lines.count }) lines")
                }
            }
        }
