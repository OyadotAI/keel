...
///
/// The primary action grants only this invocation. Longer-lived permissions stay available,
/// with their scope spelled out, rather than being the easiest button to press accidentally.
struct ApprovalCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: K.S.md) {
            ...
            HStack(spacing: K.S.sm) {
                Button("Allow once") { model.answer(pending, allow: true, scope: "once") }
                    .buttonStyle(FilledButton())
                    .help("Allow only this invocation. No rule is saved. (⌘⇧A)")
                Menu("More options") {
                    Button("Allow for this session") {
                        model.answer(pending, allow: true, scope: "session")
                    }
                    Button(allowLabel + " in this project") {
                        model.answer(pending, allow: true, scope: "project")
                    }
                    Divider()
                    Button("Trust all commands in this project") {
                ...
                .help("Choose a longer-lived permission explicitly")
                Spacer(minLength: 0)
                Button("Deny") { model.answer(pending, allow: false, scope: "session") }
                    .buttonStyle(QuietButton(tone: K.C.del))
                    .help("Refuse this command (⌘⇧D)")
            }
            Text("Allow once runs only this command. Nothing is remembered.")
