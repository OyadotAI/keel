...
final class Lanes {
    ...
    /// Whether this window's tabs are the ones written down for the project — see `remember`.
    ///
    /// Exactly one window can be, because the saved list is keyed by project alone. A torn-out
    /// window that also restored it came up holding the *other* window's tabs instead of the one
    /// lane it was given, and both then wrote the key on every change with the last one winning.
    ///
    /// It lives here rather than in the view because `switchProject` restores too: a guard at each
    /// call site is a guard that the next call site forgets.
    let remembers: Bool
    ...
    init(client: Client, port: UInt16, remembers: Bool = true, initialID: UUID = UUID()) {
        self.client = client
        self.port = port
        self.remembers = remembers
        project = Project(client: client, port: port)
        lanes = []
        let first = SessionModel(client: client, port: port, id: initialID, project: project)
        first.lanes = self
        lanes = [first]
        activeID = first.id
    ...
    private func watchProject() {
        let events = project.events.subscribe()
        eventsTask = Task { [weak self] in
            for await event in events {
                // The subscription can wait forever; only retain the window while routing a frame.
    ...
    /// End the whole window's work, including transcript tails and project event subscriptions.
    func shutdown() {
    ...
    private func reconnected() async {
        ...
        await refreshWorktrees()
        guard isCurrent(mine) else { return }
        // Events lost during a disconnect can belong to any open checkout. Shared checkouts
        // are read once even when several lanes use them.
        let checkouts: [String?] = [nil] + Set(lanes.compactMap(\.worktree)).sorted().map { Optional($0) }
        for checkout in checkouts {
            let repo = project.repo(for: checkout)
            _ = await repo.refreshGit(project.client)
            guard isCurrent(mine) else { return }
            _ = await repo.refreshTree(project.client)
            guard isCurrent(mine) else { return }
            await repo.refreshBranches(project.client)
            guard isCurrent(mine) else { return }
        }
        await a.refreshTrust()
        guard isCurrent(mine) else { return }
        for lane in lanes {
            lane.sessionsChanged(project.store.sessions)
    ...
    func restore(repo: String) async {
        guard !stopped, !Task.isCancelled, remembers, !repo.isEmpty,
        ...
        var adopted = false
        defer {
            if !adopted { for lane in restored { lane.closed() } }
        }
        ...
        guard isCurrent(mine), !restored.isEmpty else { return }
        for lane in lanes { lane.closed() }
        lanes = restored
        adopted = true
