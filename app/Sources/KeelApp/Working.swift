...
struct WorkingBar: View {
    ...
    private var activity: String {
        ...
        // carries one fact when the gate starts and nothing until it ends, so this read as the
        // last tool call "quiet for 2m" through a three-minute `make check`. The fact that opened
        // the gate is the last progress, so `quiet` is how long it has been running.
        if case .running(let command) = turn.gate {
            return quiet >= 60
                ? "checking — \(command), \(quiet / 60)m \(quiet % 60)s"
                : "checking — \(command), \(quiet)s"
        }
