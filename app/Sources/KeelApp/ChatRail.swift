...
struct ChatRail: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            // The banners are their own stack so the animations below belong to *them*.
            VStack(spacing: 0) {
            ...
            // Scoped to the three bars above, and deliberately not to the rail.
            //
            // An `.animation(_:value:)` on a container animates every animatable change in its
            // whole subtree that lands in the same transaction — and on the rail's root that
            // subtree is the transcript. So a turn starting or ending, which flips `running` at
            // the same moment a turn is appended, animated every materialised row for the length
            // of the animation: a full layout pass of the rail per frame, an NSTextField measured
    ...
    private func transcript(_ proxy: ScrollViewProxy) -> some View {
        ...
        // jump: a send, or a session opened, means the end is what you came for.
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: K.S.half) {
            ...
            HStack(spacing: K.S.sm) {
                ...
                ChatTurnCopyButtons(turn: turn, hovering: hovering, copied: copied, put: put)
                Spacer()
            }
        ...
        .contextMenu {
            ChatTurnMenu(turn: turn, model: model, put: put)
        }
    }
...
/// A parsed block owns its streaming dependency. The surrounding turn only observes changes
...
/// subscribed the whole row to every token, bypassing Block's 80 ms rendering coalescer.
private struct ChatTurnCopyButtons: View {
    let turn: Turn
    let hovering: Bool
    let copied: String?
    let put: (String, String) -> Void
...
private struct ChatTurnMenu: View {
    let turn: Turn
    let model: SessionModel
    let put: (String, String) -> Void
