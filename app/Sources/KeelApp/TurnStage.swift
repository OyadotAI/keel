        ...
        VStack(alignment: .leading, spacing: K.S.sm) {
            ...
            if let why = turn.notIsolated {
                ...
                HStack(alignment: .top, spacing: K.S.sm) {
                    ...
                    // A lane's turn was meant to be isolated and could not be; a followed
                    // turn was never going to be, and what it says is why nothing was checked
                    // or committed.
                    Text(turn.replayed
                         ? "Nothing was checked or committed — \(why)"
                         : "Ran in the project rather than an isolated checkout — \(why)")
...
struct ChangedFiles: View {
    ...
    /// Deduplicated against the normalised path, so an absolute path from an `Edit` and a
    /// relative path from `git` status never produce duplicate cards for the same file.
    private var distinctFiles: [(path: String, shown: String)] {
        var seen = Set<String>()
        var result: [(path: String, shown: String)] = []
        for path in turn.files {
            let shown = model.repoRelative(path)
            if seen.insert(shown).inserted {
                result.append((path: path, shown: shown))
            }
        }
        return result
    }
    ...
    private var outside: Int {
        distinctFiles.count { $0.shown.hasPrefix("/") }
    }

    private var title: String {
        let n = distinctFiles.count
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            if open {
                VStack(alignment: .leading, spacing: K.S.sm) {
                    ...
                    ForEach(distinctFiles, id: \.shown) { file in
                        ...
                        if file.shown.hasPrefix("/") {
                            OutsideFile(path: file.shown, model: model)
                        } else {
                            FileDiff(path: file.shown, model: model)
                        }
                    }
                }
...
struct RawOutput: View {
    ...
    /// while it is running these lines are the live evidence of work, and once done the clean
    /// summary stands without a wall of raw records.
