...
struct Template: Identifiable, Hashable {
    ...
    let icon: String
    let blurb: String
    /// The scaffold to lay down: `app` (Cloudflare Workers) or `empty` (agent scaffolding only).
    /// The gallery replaces `app` with `stack` when Containers is chosen.
    let scaffold: String
    ...
    static let all: [Template] = [
        Template(id: "fullstack", title: "Full-stack app", icon: "square.stack.3d.up",
                 blurb: "Next.js front, Hono API, Postgres, two environments. The scaffold as-is.",
                 scaffold: "app", brief: "", wants: nil),
        Template(id: "api", title: "REST API service", icon: "point.3.connected.trianglepath.dotted",
                 blurb: "Typed Hono routes, API keys, rate limits, OpenAPI docs.",
                 scaffold: "app",
        ...
        Template(id: "jobs", title: "Webhooks & background jobs", icon: "arrow.triangle.2.circlepath.circle",
                 blurb: "Inbound webhooks verified and queued, workers that retry, scheduled jobs.",
                 scaffold: "app",
                 brief: "Build an event-processing backend: webhook endpoints that verify signatures (Stripe and GitHub as examples) and enqueue to Redis, a consumer with idempotency keys in the database and exponential retry, a scheduled job runner, a dead-letter table, and an admin page listing events and their state. Run the gate when done.",
                 wants: nil),
        ...
        Template(id: "tenant", title: "Multi-tenant SaaS backend", icon: "building.2",
                 ...
                 scaffold: "app",
                 brief: "Build a multi-tenant SaaS backend: organisations and memberships in the database, every table scoped by org id with a query layer that cannot forget the scope, invitations, plan limits enforced in middleware, Stripe webhook handling for subscription state, and an org settings page. Run the gate when done.",
                 wants: nil),
        Template(id: "pipeline", title: "Data pipeline", icon: "arrow.down.right.and.arrow.up.left",
                 blurb: "Ingest raw records, transform on a schedule, aggregate, expose an API.",
                 scaffold: "app",
                 brief: "Build a data pipeline: an ingest endpoint that appends raw records to an events table, a scheduled job that transforms new events and upserts daily aggregates, a backfill command, a metrics API over the aggregates, and a frontend page with a chart and a table. Make transforms pure functions with unit tests. Run the gate when done.",
                 wants: nil),
        Template(id: "agent", title: "AI agent backend", icon: "sparkles",
                 ...
                 scaffold: "app",
                 brief: "Build an AI agent backend: a Hono endpoint that streams Claude API responses with tool use (a web-fetch tool and a notes tool backed by the database), conversation history in the database, a system prompt editable from a settings page, per-user rate limits, and a minimal streaming chat UI to exercise it. The API key comes from the environment, never in the repository. Run the gate when done.",
                 wants: nil),
        Template(id: "realtime", title: "Realtime service", icon: "dot.radiowaves.left.and.right",
                 blurb: "WebSockets: rooms, presence, ordered messages.",
                 scaffold: "app",
                 brief: "Build a realtime service: a WebSocket endpoint with a room per channel, presence, ordered message history persisted in the database, Redis pub/sub so rooms work across replicas, a Hono route that issues short-lived join tokens, and a frontend page that joins a room and shows messages live. Run the gate when done.",
                 wants: nil),
