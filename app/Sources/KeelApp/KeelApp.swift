    ...
    var body: some Scene {
        ...
        .commands {
            ...
            CommandMenu("Agent") {
                ...
                Divider()
                Button("Allow oldest command once") {
...
final class AppModel {
    ...
    func workspace(for request: Detached) -> Workspace {
        ...
        let placeholder = Workspace(port: 0, request: request)
        detached[request.lane] = placeholder
        detachedStarts[request.lane] = Task { @MainActor in
            var current = placeholder
            defer {
                // A closed lane can be reopened before this cancelled task unwinds.
                if detached[request.lane] === current {
                    detachedStarts.removeValue(forKey: request.lane)
                }
            }
            ...
            let real = Workspace(port: port, request: request)
    ...
    func start() async {
        guard !started, !stopped, !Task.isCancelled else { return }
        started = true
        Telemetry.start()
        ...
        do { try await daemon.start() } catch {
    ...
    private func advertiseIfReachable() async {
        ...
        guard !stopped, !Task.isCancelled, !devices.isEmpty else { return }
