...
struct ChangesTreeView: View {
    ...
    var body: some View {
        if !model.isRepo, model.editedThisSession.isEmpty {
    ...
    private var repoContents: some View {
        ...
        if let notice = SessionModel.droppedNotice(model.replayDropped, bytes: model.replayDroppedBytes) {
            Text("File list covers the loaded history. " + notice)
                .font(K.F.micro).foregroundStyle(K.C.dim).padding(K.S.md)
        }
        if edited.isEmpty {
            EmptyState(icon: "plusminus", title: "No recorded file changes",
                       model.turns.isEmpty
                       ? "Every file the agent writes in this conversation appears here, as a diff."
                       : "No file edits were recovered from the loaded records. Commands and tool output are still available in the trace.")
        } else {
            ForEach(ChangeTree.build(edited.filter { !$0.path.hasPrefix("/") })) { node in
                ChangeRow(node: node, depth: 0, model: model)
            }
            let outside = edited.filter { $0.path.hasPrefix("/") }
            if !outside.isEmpty {
                Text("Outside the opened project")
                    .font(K.F.micro).foregroundStyle(K.C.dim).padding(K.S.md)
                ForEach(outside, id: \.path) { change in
                    ChangeRow(node: ChangeTree.Node(name: change.path, path: change.path,
                                                   isDir: false, change: change), depth: 0, model: model)
                }
            }
        }
    }
...
struct ChangeRow: View {
    ...
    var body: some View {
        if node.isDir {
            ...
            .help(node.path)
            .contextMenu {
                if !node.path.hasPrefix("/"), model.isRepo {
                ...
                Button("Reveal in Finder") {
                    if node.path.hasPrefix("/") {
                        ...
                        Task { await model.reveal(node.path) }
                    }
                }
    ...
    private var tint: Color {
        ...
        if s.hasPrefix("C") || s.hasPrefix("H") { return K.C.faint }
        return K.C.warn
    }
}
...
private struct CommitFile: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            if open {
                Hairline()
                let (shown, dropped) = diff.preview()
                ForEach(Array(shown.enumerated()), id: \.offset) { _, hunk in
                ...
                // Every file in a commit opens at once, and this stack is not lazy.
                if dropped > 0 {
                    Text("…\(dropped) more line\(dropped == 1 ? "" : "s"), not shown")
