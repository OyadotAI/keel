...
enum K {
    ...
    enum C {
        ...
        static let all: [(String, Color)] = [
            ...
            ("accent", accent), ("onAccent", onAccent), ("add", add), ("del", del), ("warn", warn),
...
struct FilledButton: ButtonStyle {
    ...
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .lineLimit(1)
            .foregroundStyle(enabled ? K.C.onAccent : K.C.faint)
            .padding(.horizontal, K.S.md)
...
/// turn streamed out of sight. Only a drag, a wheel or a fling can unfollow, and settling back
/// at the end follows again. A gesture owns the offset until it ends, including deceleration.
    ...
    func body(content: Content) -> some View {
        content
            .onScrollPhaseChange { _, phase, context in
                ...
                // Release before the first scroll delta, not after geometry crosses a threshold.
                // Recheck when the gesture ends too: it may already be at the tail, so there is
                // no further Boolean geometry transition to switch following back on.
                follow(atTail: atTail(context.geometry), byHand: byHand)
            }
            .onScrollGeometryChange(for: Bool.self) { g in
                ...
                follow(atTail: atBottom, byHand: byHand)
            }
    }

    private func atTail(_ geometry: ScrollGeometry) -> Bool {
        top ? geometry.contentOffset.y <= 24
            : geometry.contentOffset.y + geometry.containerSize.height >= geometry.contentSize.height - 24
    }

    private func follow(atTail: Bool, byHand: Bool) {
        let next = Self.following(pinned, atBottom: atTail, byHand: byHand)
