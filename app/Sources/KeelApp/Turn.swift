...
final class Turn: Identifiable {
    ...
    var thinking = ""
    @ObservationIgnored var workingDirectory: String?
    ...
    func noteEdit(_ path: String?) {
        ...
        // A suffix match can be a different file in another checkout. Consumers that know the
        // checkout normalize absolute/relative aliases; a turn must not throw away evidence.
        guard !files.contains(clean) else { return }
        files.append(clean)
    }
    ...
    private func noteShellWrites(_ call: Call) {
        ...
        if !call.failed {
            for path in RecordedFileWrites.paths(in: command, cwd: call.cwd) { noteEdit(path) }
        }
        // A replayed call's `started` is now, not when the command ran. Today's mtimes cannot
        // establish what happened in that historical session (and can blame unrelated edits).
        if !replayed {
    ...
    static func subject(of input: [String: JSONValue]) -> String {
        let subject = ["command", "file_path", "path", "notebook_path", "pattern", "description"]
