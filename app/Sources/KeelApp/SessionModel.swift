...
final class SessionModel: Identifiable {
    ...
    var editedThisSession: [Wire.Change] {
        ...
        for turn in turns {
            // The same source as the turn's file cards: full paths, nested agent writes, shell
            // writes and daemon facts. `Call.subject` is only a truncated display label.
            for recorded in turn.files {
                let path = repoRelative(recorded)
                guard !path.isEmpty else { continue }
                if turn.replayed { historical.insert(path) }
                // Keep the live scratch-file cleanup, but never erase history because a file
                // was renamed, deleted, or belongs to a checkout that no longer exists.
                if !historical.contains(path), recorded.hasPrefix("/"),
                   pending[path] == nil,
                   !FileManager.default.fileExists(atPath: recorded) {
                    seen[path] = nil
                    continue
                }
                if ordered.insert(path).inserted { order.append(path) }
                seen[path] = Wire.Change(path: path, status: "M", label: "edited")
            }
        }
        ...
        return order.compactMap { path in
            ...
            } else if historical.contains(path) {
                // Absence from today's git status does not prove an old edit was committed.
                change.status = "H"
                change.label = "recorded"
            } else {
                change.status = "C"
    ...
    /// A lane-scoped Stop must reach the daemon before a replacement takes that lane.
    ...
    func send() {
        ...
        guard !running, !settling else { queued.append(text); return }
        start(text)
    }
    ...
    private func start(_ text: String) {
        ...
        streamTask = Task { [client] in
            ...
            guard !Task.isCancelled, self.current === turn else { return }
            self.stopTask = nil
            ...
            // Cancellation can end the sequence normally. Neither path may finish a newer turn.
            guard !Task.isCancelled, self.current === turn else { return }
            ...
            if turn.failed, self.sessionId == nil, !self.retriedFresh, turn.files.isEmpty,
               Self.sessionIsGone(turn.failure ?? "") {
    ...
    func stop() {
        ...
        let stoppedTurn = current
        let earlierStop = stopTask
        stopTask = Task { [client, weak self] in
            ...
            do {
                ...
                if let self, self.current === stoppedTurn {
    ...
    struct Record: Decodable {
        ...
        /// Resolve literal shell-write paths against the directory recorded at execution time.
        var cwd: String?
    ...
    private func absorb(_ r: Record, raw data: Data, into turn: Turn) {
        if let cwd = r.cwd, !cwd.isEmpty {
            turn.workingDirectory = cwd
        } else if turn.workingDirectory == nil {
            turn.workingDirectory = sessionCwd ?? (repoPath.isEmpty ? nil : repoPath)
        }
    ...
    func refreshMonitors() async {
        ...
        guard let jobs: [Wire.Job] = try? await client.get("/api/monitors", q), !isClosed else { return }
    ...
    func fetchApprovals() async {
        ...
        // Poll removes these cards from the daemon's queue. If their turn stopped while the
        // request was in flight, release its waiters without resurrecting cards in this lane.
        guard running, current === turn else {
            for card in found { answer(card, allow: false, scope: "once", restoreOnFailure: false) }
            return
        }
    ...
    func answer(_ p: Wire.Pending, text: String) {
        ...
        Task { [client] in
            do {
                ...
                if self.running, self.current === turn { self.answerFailed(p, error) }
            }
        }
    }
    ...
    func answer(_ p: Wire.Pending, allow: Bool, scope: String, restoreOnFailure: Bool = true) {
        let turn = current
        pending.removeAll { $0.id == p.id }
        ...
        Task { [client] in
            do {
                ...
                if restoreOnFailure, self.running, self.current === turn { self.answerFailed(p, error) }
                return
            }
    ...
    func open(session id: String, cwd: String? = nil) async {
        sessionId = id
        let known = sessions.first { $0.id == id }
        title = known?.title ?? "Session " + id.prefix(8)
        sessionCwd = cwd ?? (known?.elsewhere != nil ? known?.cwd : nil)
