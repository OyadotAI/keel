//! The Cloudflare golden path: what a new project starts with.
//!
//! Three folders, because the halves have genuinely different constraints — `frontend/` (Next.js
//! compiled to a Worker by OpenNext), `backend/` (Hono, its own Worker) and `infra/`. The
//! frontend reaches the backend through a service binding, so the call never leaves Cloudflare
//! and the backend needs no public route.
//!
//! This was a thousand lines inside the daemon's `POST /api/project` handler. The handler
//! validated a name, made a directory and then carried the entire template; what is left there
//! now is the handler.
...
///
/// It lives with the template rather than with the handler that calls it: what a name is allowed
/// to be is a fact about Cloudflare and about filesystems, not about HTTP.
