...
pub enum Status {
    Busy,
    Idle,
    /// A live process that has not said. Not idle: a follower finishing a turn on this would
    /// be finishing it under a `claude` mid-write.
    Unknown,
}
...
pub fn status(claude_home: &Utf8Path, session: &str) -> Option<Status> {
    running(claude_home).get(session).map(|busy| match busy {
        Some(true) => Status::Busy,
        Some(false) => Status::Idle,
        None => Status::Unknown,
    })
}
...
/// Live sessions by id, with whether each is busy — `None` when the pid file has no `status`,
/// which is the case until Claude Code's first status update: a freshly started or re-attached
/// process says nothing either way, and reading that as "idle" listed a working session as idle
/// and, worse, told the follower a turn was over.
fn running(claude_home: &Utf8Path) -> std::collections::HashMap<String, Option<bool>> {
    ...
    type Parsed = Option<(i32, String, Option<bool>)>;
            ...
            None => {
                let p: Parsed = std::fs::read_to_string(&path)
                    ...
                    .and_then(|record| {
                        ...
                        let busy = record["status"].as_str().map(|s| s == "busy");
                        Some((pid, id, busy))
                    });
                memo.lock()
        ...
        let project = p.parent().map(|d| d.to_owned()).unwrap_or_default();
        {
            if let Some(mut s) = parse_session(&p) {
                ...
                if let Some(busy) = running.get(&s.id) {
                    s.live = true;
                    // No word from the process: the transcript's own recency stands in. Thirty
                    // seconds covers a turn between tool calls; a quiet session past that is
                    // waiting on somebody.
                    s.busy = busy.unwrap_or_else(|| {
                        at.elapsed()
                            .is_ok_and(|since| since < std::time::Duration::from_secs(30))
                    });
                }
                sessions.push(s);
            }
...
mod tests {
    ...
    mod following {
        ...
        /// A pid file with no `status` is a process that has not said, not one that is idle.
        #[test]
        fn a_pid_file_without_a_status_is_not_idle() {
            let (_d, home) = home_with("-repo", "abc-1.jsonl", &format!("{}\n", user("one")));
            let sessions_dir = home.join("sessions");
            std::fs::create_dir_all(&sessions_dir).unwrap();
            let me = std::process::id();
            std::fs::write(
                sessions_dir.join(format!("{me}.json")),
                format!(r#"{{"pid":{me},"sessionId":"abc-1"}}"#),
            )
            .unwrap();
            assert_eq!(status(&home, "abc-1"), Some(Status::Unknown));
            let listed = &discover_sessions(Utf8Path::new("/repo"), &home)[0];
            assert!(listed.live);
            assert!(
                listed.busy,
                "just written, so working until it says otherwise"
            );
        }
