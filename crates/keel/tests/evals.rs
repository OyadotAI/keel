...
fn stubbed_project() -> (tempfile::TempDir, u16, Child, std::path::PathBuf) {
    ...
    let daemon = Command::new(env!("CARGO_BIN_EXE_keel"))
        ...
        .env("SHELL", &shell)
        .env("KEEL_PERMISSIONS_DIR", permission_test_storage())
        .stdout(Stdio::null())
        .stderr(Stdio::null())
        .spawn()
        .expect("daemon");
...
///
/// "Trust this project" is stored outside the repository on this Mac, and that
...
///
/// A trusted project's command needs no additional lifetime question. "Should this keep
...
/// Shared across test daemons but never written into a fixture repository or the user's HOME.
fn permission_test_storage() -> std::path::PathBuf {
