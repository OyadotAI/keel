...
fn parse_entry(entry: &str) -> Option<Change> {
    ...
    let path = entry.get(3..).filter(|path| !path.is_empty())?;
    Some(Change {
...
fn parse_status(raw: &str) -> Vec<Change> {
    ...
    while let Some(record) = records.next() {
        let Some(change) = parse_entry(record) else {
            continue;
        };
        // Porcelain -z emits `XY destination\0source\0` for renames and copies. The source
        // record has no status prefix and must never be parsed as a second changed file.
        if change.status.contains(['R', 'C']) {
            records.next();
        }
        if change.status != "??" || !is_dependency(&change.path) {
            changes.push(change);
        }
    }
    changes
...
fn git_status_in(root: &Utf8Path) -> GitStatus {
    ...
    let mut changes = parse_status(&raw);
    ...
    if changes.len() > MAX_CHANGES {
        collapsed = true;
        changes = parse_status(&git(root, &["status", "--porcelain=v1", "-z"]).unwrap_or_default());
...
mod git_tests {
    ...
    fn status_preserves_leading_spaces_and_consumes_rename_source_records() {
        ...
        std::fs::write(root.join("日本語.txt"), "rename me\n").unwrap();
        git_run(&root, &["add", "-A"]).unwrap();
        git_run(&root, &["commit", "-qm", "seed"]).unwrap();
        git_run(&root, &["mv", "日本語.txt", " renamed.txt"]).unwrap();
        std::fs::write(root.join(" untracked.txt"), "new\n").unwrap();

        let status = git_status(&root);
        assert_eq!(status.changes.len(), 2, "a rename has one row");
        assert!(
            status
                .changes
                .iter()
                .any(|c| c.path == " renamed.txt" && c.status == "R ")
        );
        assert!(
            status
                .changes
                .iter()
                .any(|c| c.path == " untracked.txt" && c.status == "??")
        );
    }

    #[test]
    fn tracked_dependency_edits_are_visible_without_staging() {
        let dir = tempfile::tempdir().unwrap();
        let root = Utf8PathBuf::from_path_buf(dir.path().to_path_buf()).unwrap();
        for args in [
            &["init", "-q"][..],
        ...
        std::fs::create_dir(root.join("Pods")).unwrap();
        std::fs::write(root.join("Pods/tracked.swift"), "original\n").unwrap();
        git_run(&root, &["add", "-A"]).unwrap();
        git_run(&root, &["commit", "-qm", "seed"]).unwrap();
        std::fs::write(root.join("Pods/tracked.swift"), "edited\n").unwrap();
        std::fs::write(root.join("Pods/untracked.swift"), "dependency\n").unwrap();

        let status = git_status(&root);
        assert_eq!(status.changes.len(), 1);
        assert_eq!(status.changes[0].path, "Pods/tracked.swift");
