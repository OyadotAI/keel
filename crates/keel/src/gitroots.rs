...
mod tests {
    ...
    #[test]
    fn partial_workspace_commits_report_successes_and_failures_and_can_be_retried() {
        for failed in ["backend", "frontend"] {
            let dir = tempfile::tempdir().unwrap();
            let root = Utf8Path::from_path(dir.path()).unwrap();
            let saved = if failed == "backend" {
                "frontend"
            } else {
                "backend"
            };
            for name in ["backend", "frontend"] {
                repo(&root.join(name));
                std::fs::write(root.join(name).join("new.txt"), "work to save\n").unwrap();
            }
            let git =
                |name: &str, args: &[&str]| crate::git::trimmed(&root.join(name), args).unwrap();
            let original_saved = git(saved, &["rev-parse", "HEAD"]);
            let original_failed = git(failed, &["rev-parse", "HEAD"]);
            let lock = root.join(failed).join(".git/index.lock");
            std::fs::write(&lock, "another Git operation owns the index\n").unwrap();

            let error = crate::worktree::commit_all(root, "save both repositories").unwrap_err();
            assert!(error.contains(&format!("Committed: {saved}.")), "{error}");
            assert!(
                error.contains(&format!("Failed to commit: {failed}:")),
                "{error}"
            );
            assert!(
                error.contains("index.lock"),
                "the Git failure must be included: {error}"
            );
            let committed = git(saved, &["rev-parse", "HEAD"]);
            assert_ne!(
                committed, original_saved,
                "the other repository still commits"
            );
            assert_eq!(git(failed, &["rev-parse", "HEAD"]), original_failed);
            assert_eq!(
                std::fs::read_to_string(root.join(failed).join("new.txt")).unwrap(),
                "work to save\n"
            );

            std::fs::remove_file(lock).unwrap();
            assert_eq!(
                crate::worktree::commit_all(root, "save both repositories"),
                Ok(true)
            );
            assert_eq!(
                git(saved, &["rev-parse", "HEAD"]),
                committed,
                "retry does not duplicate a saved commit"
            );
            assert_ne!(git(failed, &["rev-parse", "HEAD"]), original_failed);
        }
    }
