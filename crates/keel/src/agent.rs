...
pub async fn chat(
    ...
    tokio::spawn(async move {
        ...
        let writes = query.mode.as_deref() == Some("acceptEdits");
        ...
        // This scans files and invokes toolchain detection. Keeping it on the async executor
        // could delay the Stop request for the entire preparation on a large repository.
        let base_system =
            if provider == "claude" || (provider == "codex" && query.session.is_none()) {
                let checkout = cwd.clone();
                crate::serve::blocking(move || system_prompt(&checkout), String::new()).await
            } else {
                String::new()
            };
        if preparation_stopped(&stopper, &lane, token, &tx).await {
        ...
        let pid = child.child.id().unwrap_or(0);
        if pid != 0 {
        ...
        if let (Some(session), Some(turn)) = (session, key) {
            ...
            if let Some(forwarder) = forwarder {
                // A timed-out JoinHandle detaches its task. Without aborting, its sender keeps
                // this event stream open forever if the final fact was never observed.
...
async fn quarantine_before_launch(
    ...
    if let Err(e) = quarantined {
        ...
        sentry::capture_message(
            "quarantine failed: agent launch refused",
            sentry::Level::Error,
        );
        let _ = tx
            .send(Ok(Event::default().event("fatal").data(format!(
...
/// Cancellation checkpoints keep the claim until any blocking preparation already started has
/// returned. Dropping its future early would release the tree while a snapshot still writes it.
async fn preparation_stopped(
    state: &AppState,
    lane: &str,
    token: u64,
    tx: &tokio::sync::mpsc::Sender<Result<Event, Infallible>>,
...
/// Task cancellation and unwinding must stop descendants as well as the provider. Tokio reaps
/// the child on drop; normal disconnects explicitly wait before giving the checkout back.
struct AgentChild {
...
async fn next_agent_line<R: tokio::io::AsyncBufRead + Unpin>(
    ...
    tokio::select! {
        ...
        next = crate::lines::next(lines) => Some(next),
    }
}
...
async fn wait_for_agent(
    ...
    tokio::select! {
        biased;
        _ = tx.closed() => child.terminate().await,
...
mod stream_tests {
    ...
    async fn quarantine_result(root: &Utf8Path) -> (bool, String) {
        let state = Arc::new(AppState::new(root.to_owned()));
        let token = state.claim("quarantine", root, true).unwrap();
        let (tx, rx) = tokio::sync::mpsc::channel(1);
        let allowed = {
            let _held = crate::serve::Held::new(state.clone(), "quarantine".into(), token);
            quarantine_before_launch(root.to_owned(), &tx).await
        };
    ...
    #[tokio::test]
    async fn quarantine_failure_refuses_launch_and_releases_the_lane() {
        let dir = tempfile::tempdir().unwrap();
        let root = Utf8Path::from_path(dir.path()).unwrap();
        std::fs::create_dir(root.join(".claude")).unwrap();
        std::fs::write(root.join(".claude/settings.json"), "{\"hooks\":{}}\n").unwrap();
        // A file at the destination makes quarantine fail deterministically, even as root.
        std::fs::create_dir(root.join(".keel")).unwrap();
        std::fs::write(root.join(".keel/quarantine"), "preserve me\n").unwrap();

        let (allowed, events) = quarantine_result(root).await;
        assert!(
            !allowed,
            "a failed quarantine still allowed the agent to start"
        );
        assert!(events.contains("event: fatal"), "{events}");
        assert!(events.contains("not started"), "{events}");
        assert!(root.join(".claude/settings.json").exists());
        assert_eq!(
            std::fs::read_to_string(root.join(".keel/quarantine")).unwrap(),
            "preserve me\n"
        );
    }
    ...
    async fn quarantine_collision_refuses_launch_and_preserves_both_copies() {
        ...
        let root = Utf8Path::from_path(dir.path()).unwrap();
        std::fs::create_dir(root.join(".claude")).unwrap();
        std::fs::write(root.join(".claude/settings.json"), "original\n").unwrap();
    ...
    #[tokio::test]
    async fn quarantine_allows_a_clean_checkout_and_successfully_moved_config() {
        let dir = tempfile::tempdir().unwrap();
        let root = Utf8Path::from_path(dir.path()).unwrap();
        let (allowed, events) = quarantine_result(root).await;
        assert!(allowed);
    ...
    #[tokio::test]
    async fn cancelled_preparation_never_reaches_spawn() {
        ...
        assert!(
            ...
            "the client needs a terminal event"
        );
    ...
    async fn disconnect_stops_a_silent_agent_before_releasing_its_checkout() {
        ...
        let dir = tempfile::tempdir().unwrap();
        let checkout = Utf8Path::from_path(dir.path()).unwrap();
        let token = state.claim("silent", checkout, true).unwrap();
        let held = crate::serve::Held::new(state.clone(), "silent".into(), token);
        let mut child = silent_agent(false);
        state.started("silent", token, child.child.id().unwrap());
        let mut lines = BufReader::new(child.child.stdout.take().unwrap()).lines();
        let (tx, rx) = tokio::sync::mpsc::channel(1);
        assert!(matches!(
            next_agent_line(&mut lines, &mut child, &tx).await,
            Some(crate::lines::Next::Line(line)) if line == "ready"
        ));
        assert!(state.claim("replacement", checkout, true).is_err());

        // Let the output reader start waiting before the remote end closes.
        tokio::spawn(async move {
        ...
        let result = tokio::time::timeout(
            ...
            next_agent_line(&mut lines, &mut child, &tx),
        )
        .await;
        assert!(
            matches!(result, Ok(None)),
            "the disconnected turn kept waiting for output from a silent agent"
        );
        assert!(child.child.id().is_none(), "the child must be reaped first");
        assert!(state.claim("replacement", checkout, true).is_err());
        drop(held);
    ...
    async fn disconnect_after_stdout_eof_still_reaps_the_agent() {
        ...
        let result = tokio::time::timeout(
            std::time::Duration::from_secs(2),
            wait_for_agent(&mut child, &tx),
        )
        .await;
        assert!(
            result.is_ok_and(|status| status.is_ok_and(|status| !status.success())),
            "closing stdout is not proof that the agent exited"
        );
