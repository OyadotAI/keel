...
fn well_known() -> Vec<String> {
    ...
    .into_iter()
    // Keep locations that do not exist yet: first-run setup may create ~/.local/bin after
    // startup, and the freshly installed CLI must be usable without restarting the app.
    .collect()
}
