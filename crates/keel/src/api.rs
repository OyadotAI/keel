...
///
/// The preview navigates to the page a written file *is*. A component is not a page — editing
/// `components/domain-step.tsx` changes a screen somewhere, and the preview used to sit on
/// whatever it was already showing while the status bar said "Editing domain-step.tsx…". The
/// answer is one level up the import graph, repeated until a file that is a route is reached, and
/// the client does that walk by asking this repeatedly.
///
/// Text search rather than an AST or a bundler. An import specifier ends in the module's own name,
/// which is enough to walk the graph, and a wrong answer is cheap: the caller navigates only when
/// the walk finds exactly one route. Ignore rules come from the same walker the tree uses, so
/// `node_modules` and build output are never read.
