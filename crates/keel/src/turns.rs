...
mod tests {
    ...
    fn scratch() -> Utf8PathBuf {
        ...
        let nanos = std::time::SystemTime::now()
            ...
            .map(|d| d.as_nanos())
            .unwrap_or(0);
        let temp = Utf8PathBuf::from(std::env::temp_dir().to_string_lossy().to_string());
        // Callers hold a bare path, so nothing drops these; 722 of them had piled up at one per
        // test per `make check`. Each call sweeps the ones from earlier runs instead.
        // ponytail: an age sweep rather than a TempDir guard in 21 call sites.
        if let Ok(entries) = std::fs::read_dir(&temp) {
            for e in entries.flatten() {
                let old = e.file_name().to_string_lossy().starts_with("keel-turns-")
                    && e.metadata()
