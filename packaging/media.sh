...
mkdir -p "$out"
for name in turn approve workflow; do
    ...
    width=900
    if [ "$name" = workflow ]; then width=1200; fi
    ffmpeg -y -loglevel error -framerate 10 -i "$work/frames/$name/%03d.png" \
        -filter_complex "fps=10,scale=$width:-1:flags=lanczos,split[s0][s1];[s0]palettegen=max_colors=128[p];[s1][p]paletteuse=dither=bayer:bayer_scale=3" \
        -loop 0 "$out/$name.gif"
    # The same honest demo in a social-upload friendly container. No audio or tracking metadata.
    ffmpeg -y -loglevel error -framerate 10 -i "$work/frames/$name/%03d.png" \
        -vf "scale=$width:-2:flags=lanczos,format=yuv420p" \
        -c:v libx264 -crf 20 -movflags +faststart -an "$out/$name.mp4"
done

for still in turn approval workspace commands setup installers; do
